...
import HeaderRightShimmer from "./loadingEffect/HeaderRightShimmer ";
...
const Header = () => {
  ...
  const isLoading = useSelector((state) => state?.user?.fetchingUserLoading);
  ...
  return (
    <>
      ...
      {userDrawer && (
        <div
          className="sm:hidden fixed inset-0 bg-black/40 z-[999]"
          onClick={() => setUserDrawer(false)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="absolute right-0 top-0 h-full w-64 bg-blue-600 text-white p-5 shadow-xl animate-slideLeft"
          >
            <UserProfile setUserDrawer={setUserDrawer} />
          </div>
        </div>
      )}
    </>
